...
**Tech Stack:** Python 3.12, PyObjC (Cocoa/Quartz), `rumps`, `httpx`, Pillow, LM Studio via `core.llm`, `unittest` (stdlib) for tests, run through `uv`.
...
- Tests are `unittest.TestCase` classes in `backend/tests/`, matching `backend/tests/test_menubar.py`. No pytest fixtures, no third-party test libraries.
- The project has no pytest. Run one module with `uv run --python 3.12 python -m unittest tests.<module> -v` and the suite with `make backend-test` (`uv run --python 3.12 python -m unittest discover -s tests`). Baseline before this work: 164 tests passing.
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_models -v`
...
Run: `uv run --python 3.12 python -m unittest discover -s tests`
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_classifier -v`
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_state -v`
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_context -v`
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_voice -v`
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_watcher -v`
...
```bash
uv run --python 3.12 python -c "from pet import watcher; print(watcher.sample(at=0.0))"
```
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_sprites -v`
...
Run: `uv run --python 3.12 python -m unittest tests.test_pet_runner -v`
...
```bash
uv run --python 3.12 python -m pet
```
...
- [ ] `uv run --python 3.12 python -m unittest discover -s tests` passes.
